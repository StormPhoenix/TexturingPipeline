...
//        };
        ...
        Image<T>::Ptr tmpImage = Image<T>::create(this->width(), this->height(), this->channels());

        for (int ch = 0; ch < this->channels(); ch++) {
            for (int y = 1; y < this->height() - 1; y++) {
                for (int x = 1; x < this->width() - 1; x++) {
                    double sum = (kernel[0][0] * double(this->at(x - 1, y - 1, ch)) +
                                  kernel[0][1] * double(this->at(x, y - 1, ch)) +
                                  kernel[0][2] * double(this->at(x + 1, y - 1, ch)) +
                                  kernel[1][0] * double(this->at(x - 1, y, ch)) +
                                  kernel[1][1] * double(this->at(x, y, ch)) +
                                  kernel[1][2] * double(this->at(x + 1, y, ch)) +
                                  kernel[2][0] * double(this->at(x - 1, y + 1, ch)) +
                                  kernel[2][1] * double(this->at(x, y + 1, ch)) +
                                  kernel[2][2] * double(this->at(x + 1, y + 1, ch)));
                    if (sum > 255) {
                        sum = 255;
                    }
        ...
        for (int ch = 0; ch < this->channels(); ch++) {
            for (int y = 1; y < this->height() - 1; y++) {
                for (int x = 1; x < this->width() - 1; x++) {
                    this->at(x, y, ch) = tmpImage->at(x, y, ch);
                }
            }
        }
