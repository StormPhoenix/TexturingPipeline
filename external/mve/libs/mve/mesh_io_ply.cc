...
// TODO check token amount to prevent undefined access
        ...
        load_ply_mesh(std::string const &filename) {
                ...
                if (header[0] == "format") {
                    ...
                    if (reading_verts) {
                        ...
                        if (header[1] == "list") {
                            ...
                            if (header[2] == "red") {
                                f_format.push_back(PLY_F_COLOR_RED);
                            } else if (header[2] == "green") {
                                f_format.push_back(PLY_F_COLOR_GREEN);
                            } else if (header[2] == "blue") {
                                f_format.push_back(PLY_F_COLOR_BLUE);
                            } else if (header[2] == "alpha") {
                                f_format.push_back(PLY_F_COLOR_ALPHA);
                            } else {
            ...
            TriangleMesh::ColorList &fcolors(mesh->get_face_colors());
            ...
            for (std::size_t i = 0; !eof && i < num_faces; ++i) {
                for (std::size_t n = 0; n < f_format.size(); ++n) {
                    ...
                    switch (elem) {
                        ...
                        case PLY_F_COLOR_RED: {
                            math::Vec4f face_color(0, 0, 0, 0);
                            unsigned char red = ply_read_value<unsigned char>(input, ply_format);
                            face_color[0] = red;
                            fcolors.push_back(face_color);
                            break;
                        }
                        case PLY_F_COLOR_GREEN: {
                            unsigned char green = ply_read_value<unsigned char>(input, ply_format);
                            math::Vec4f &face_color = fcolors.back();
                            face_color[1] = green;
                            break;
                        }
                        case PLY_F_COLOR_BLUE: {
                            unsigned char blue = ply_read_value<unsigned char>(input, ply_format);
                            math::Vec4f &face_color = fcolors.back();
                            face_color[2] = blue;
                            break;
                        }
                        case PLY_F_COLOR_ALPHA: {
                            unsigned char alpha = ply_read_value<unsigned char>(input, ply_format);
                            math::Vec4f &face_color = fcolors.back();
                            face_color[3] = alpha;
                            break;
                        }
            ...
            if (options.format_binary) {
                ...
                for (std::size_t i = 0; i < face_amount; ++i) {
                    ...
                    if (write_fcolors) {
                        ...
                        ply_color_convert(*fcolors[i], color, 4);
                        out.write((char const *) color, 4);
                    }
                }
            } else {
                ...
                for (std::size_t i = 0; i < face_amount; ++i) {
                    ...
                    if (write_fcolors)
                        for (int j = 0; j < 4; ++j) {
                            // TODO do not check color value range
                            float color = fcolors[i][j];
                            out << " " << (int) (color);
