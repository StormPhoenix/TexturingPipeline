...
namespace MakeDense {
    ...
    bool stop_criteria(std::vector<Face> &faces, std::vector<EdgeLen> &edge_heap) {
    ...
    void make_mesh_dense(const Base::AttributeMatrix &V, const Base::IndexMatrix &F,
        ...
        std::vector<EdgeLen> edge_len_heap;
        std::vector<EdgeFace> ef_adjacency;
            ...
            for (int i = 0; i < n_adj_face; i++) {
                {
                    ...
                    ef_adjacency.push_back(EdgeFace(new_edge));
                    ...
                    if (i == 0) {
                        edge_len_heap.back() = EdgeLen(new_edge_id, len);
                    } else {
                        edge_len_heap.push_back(EdgeLen(new_edge_id, len));
                    }
