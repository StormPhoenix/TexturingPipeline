...
Arguments parse_args(int argc, char **argv) {
    ...
    args.add_option('m', "method_type", true,
                    "Method type: {" +
                    choices<MvsTexturing::MethodType>() + "} [" +
                    choice_string<MvsTexturing::MethodType>(MvsTexturing::MRF) + "]");
    args.add_option('d', "data_term", true,
                    "Data term: {" +
                    choices<MvsTexturing::DataTerm>() + "} [" +
                    choice_string<MvsTexturing::DataTerm>(MvsTexturing::DATA_TERM_GMI) + "]");
    args.add_option('s', "smoothness_term", true,
                    "Smoothness term: {" +
                    choices<MvsTexturing::SmoothnessTerm>() + "} [" +
                    choice_string<MvsTexturing::SmoothnessTerm>(MvsTexturing::SMOOTHNESS_TERM_POTTS) + "]");
    args.add_option('o', "outlier_removal", true,
                    "Photometric outlier (pedestrians etc.) removal method: {" +
                    choices<MvsTexturing::OutlierRemoval>() +  "} [" +
                    choice_string<MvsTexturing::OutlierRemoval>(MvsTexturing::OUTLIER_REMOVAL_NONE) + "]");
    args.add_option('t',"tone_mapping", true,
                    "Tone mapping method: {" +
                    choices<MvsTexturing::ToneMapping>() +  "} [" +
                    choice_string<MvsTexturing::ToneMapping>(MvsTexturing::TONE_MAPPING_NONE) + "]");
    ...
    for (util::ArgResult const* i = args.next_option();
        ...
        switch (i->opt->sopt) {
            ...
            case 'L':
                ...
                break;
            case 'm':
                conf.settings.method_type = parse_choice<MvsTexturing::MethodType>(i->arg);
                break;
            case 'd':
                conf.settings.data_term = parse_choice<MvsTexturing::DataTerm>(i->arg);
                break;
            case 's':
                conf.settings.smoothness_term = parse_choice<MvsTexturing::SmoothnessTerm>(i->arg);
                break;
            case 'o':
                conf.settings.outlier_removal = parse_choice<MvsTexturing::OutlierRemoval>(i->arg);
                break;
            case 't':
                conf.settings.tone_mapping = parse_choice<MvsTexturing::ToneMapping>(i->arg);
                break;
            case '\0':
