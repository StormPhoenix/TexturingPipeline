...
namespace MeshPolyRefinement {
	namespace IO {
        bool read_mesh_from_memory(const std::vector<double> &vertices, const std::vector<std::size_t> &faces,
                                   Base::TriMesh &mesh);
        ...
        bool
        save_mesh_clusters(const std::string &file_name, const Base::TriMesh &mesh, const std::vector<int> &cluster_ids,
                           int num_clusters);
